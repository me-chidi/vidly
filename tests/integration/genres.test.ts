...
describe('/api/genres', () => {
    ...
    describe('GET /', () => {
        it('should return all genres', async () => {
            ...
            expect(res.body.some((g: GenreDocument) => g.name === 'genre1')).toBeTruthy();
            expect(res.body.some((g: GenreDocument) => g.name === 'genre2')).toBeTruthy();
        });
    });
    ...
    describe('PUT /:id', () => {
        let token: string;
        let name: string;
        let updateDocument: Partial<GenreDocument>;
        let options: mongoose.QueryOptions;
    ...
    describe('DELETE /:id', () => {
        ...
        beforeEach(async () => {
            token = new User({ isAdmin: true }).generateAuthToken();
            id = new mongoose.Types.ObjectId();
            genre = await Genre.create(document);
        });
